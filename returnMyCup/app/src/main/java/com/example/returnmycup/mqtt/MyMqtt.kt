package com.example.areturn.mqtt
...
class MyMqtt(context: Context, uri:String) {
    ...
    //액티비티에서 mysetCallback메소드를 호출하면서 실행할 메소드를 넘기면 MqttAndroidClient객체의 setCallback메소드가 호출되며
    //콜백메소드가 등록된다.
    fun mysetCallback(callback: (topic:String, message:MqttMessage) -> Unit){
        mqttClient.setCallback(object:MqttCallback{
            ...
            override fun messageArrived(topic: String?, message: MqttMessage?) {
                ...
                // - 액티비티의 구성하고 있는 위젯젯 메시지로 전달되는 내용을 출력하거나 사용하고
    ...
    //mqtt통신을 하기 위해 브로커서버와 연결, 연결이 끝난 후 콜백 메소드 설정
        ...
        mqttClient.connect(mqttConnectOptions, null,
                object:IMqttActionListener{
                    override fun onSuccess(asyncActionToken: IMqttToken?) {
                        ...
                        //for문이나 if문으로 처리할 수 있는데 map함수를 이용하면 편하게 작업가능
                        topic.map {
                            subscribeTopic(it)
                        }
    ...
    //토픽을 subscribe등록하기 위해서 메소드 구현 ---------------------------1)
    private fun subscribeTopic(topic:String, qos:Int = 0){
        mqttClient.subscribe(topic, qos, null, object:IMqttActionListener{
            override fun onSuccess(asyncActionToken: IMqttToken?) {
                Log.d("mymqtt", "subscibe성공")
            }

            override fun onFailure(asyncActionToken: IMqttToken?, exception: Throwable?) {
                Log.d("mymqtt", "subscibe실패")
            }
        })
    }
    ...
    fun publish(topic:String, payload:String, qos:Int=0){
        ...
        //메시지객체에 payload와 메시지전송품질를 설정
        message.payload = payload.toByteArray()
        message.qos = qos
        //메시지전송하기 (publish) - publish가 성공/실패하는 경우 이벤트가 발생하기 때문에 리스너 등록
